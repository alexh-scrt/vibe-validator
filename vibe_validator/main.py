...
@app.post(
    ...
    tags=["analysis"],
    responses={
        200: {"description": "Report HTML partial"},
        422: {"description": "Validation error HTML partial"},
        500: {"description": "Server error HTML partial"},
    },
)
async def validate(
    request: Request,
    ...
    """
    # ------------------------------------------------------------------
    # 1. Validate the incoming form data via the Pydantic model.
    # ------------------------------------------------------------------
    try:
        idea_request = IdeaRequest(idea=idea)
    except ValidationError as exc:
        # Collect all field-level error messages into a single readable string.
        messages = "; ".join(
            e["msg"] for e in exc.errors()
        )
        logger.warning("Idea validation failed: %s", messages)
    ...
    try:
        ...
        return _render_error(
            ...
            (
                ...
                "Please check that a valid OPENAI_API_KEY is configured "
        ...
        return _render_error(
            request,
            (
                "The AI returned a response we could not parse. "
                "This sometimes happens with complex ideas — please try "
                "rephrasing your idea or try again in a moment."
            ),
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )
    except ReportValidationError as exc:
        logger.error("Report validation error: %s", exc)
        return _render_error(
            request,
            (
                "The AI returned a report that did not match the expected "
                "structure. Please try again — if the issue persists, try "
                "simplifying or rephrasing your idea."
            ),
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )
    except AnalyzerError as exc:
        # Catch-all for any other AnalyzerError subclasses.
        logger.exception("Unexpected analyzer error: %s", exc)
        return _render_error(
            request,
    ...
    # ------------------------------------------------------------------
    logger.info(
        "Rendering report for idea (length=%d, score=%s)",
